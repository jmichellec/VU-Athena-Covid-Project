...
# # Read stopwords
# with open('stopwords/stops.txt', 'r', encoding='UTF-8') as f:
#     stops = f.read().split('\n')
...
stops = list(set(stopwords_all))

# Read raw data
print('reading raw data...')
#fb_preprocessed_FB_NOS_NU_Telegraaf_NRC_all_endFeb
with open('fb_preprocessed_FB_NOS_NU_Telegraaf_NRC_all_endFeb.csv', encoding='UTF-8') as csv_file:
    ...
    for row in csv_reader:
        # skip header
        if(line_count>0):
            all_timestamps_ini.append(row[9])
            all_docs_ini.append(row[-2])
        line_count += 1
...
docs = [[w.replace("covid", "corona") for w in docs[doc]] for doc in range(len(docs))]
...
trSize = int(np.floor(0.80*num_docs))
...
with open(path_save + 'vocab.txt', "w", encoding='UTF-8') as f:
    for v in vocab:
        f.write(v + '\n')

with open(path_save + 'timestamps.txt', "w", encoding='UTF-8') as f:
    for t in time_list:
        f.write(t + '\n')
