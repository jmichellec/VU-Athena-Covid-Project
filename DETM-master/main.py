#/usr/bin/python
# python main.py --dataset fb_high_sent --output_name output.txt --data_path scripts\split_paragraph_True --train_embeddings 1 --min_df 100 --num_topics 5 --lr 0.001 --epochs 5 --mode train
# python main.py --dataset fb --output_name output_0001.txt --data_path scripts\split_paragraph_True --train_embeddings 1 --min_df 0.001 --num_topics 5 --lr 0.0001 --epochs 5 --mode train
...
parser.add_argument('--emb_path', type=str, default='coosto300d.txt', help='directory containing embeddings')
parser.add_argument('--save_path', type=str, default='./results', help='path to save results')
parser.add_argument('--batch_size', type=int, default=50, help='number of documents in a batch for training')
parser.add_argument('--min_df', type=int, default=10, help='to get the right data..minimum document frequency')
parser.add_argument('--output_name', type=str, default='*.txt', help='output name .txt')
...
parser.add_argument('--eval_batch_size', type=int, default=50, help='input batch size for evaluation')
...
with open(emb_path, 'rb') as f:
    for l in f:
        line = l.decode(encoding='latin-1').split()
        word = line[0]
        if word in vocab:
        ...
        with open(args.output_name, 'w', encoding='UTF-8') as f:
            for k in range(args.num_topics):
                for t in times:
                    ...
                    f.write('Topic {} .. Time: {} ===> {}'.format(k, t, topic_words))
                    f.write('\n')

            print('\n')
            print('Visualize word embeddings ...')
            queries = ['corona', 'vaccin', 'prik', 'oma', 'oude', 'bescherming', 'bijwerkingen']
            # queries = ['covid', 'corona']
            try:
...
def get_completion_ppl(source):
    ...
    with torch.no_grad():
        ...
        if source == 'val':
            ...
            print('*'*100)
            f.write('*'*100)
            f.write('{} PPL: {}'.format(source.upper(), ppl_all))
            f.write('*'*100)
...
if args.mode == 'train':
    ...
    for epoch in range(1, args.epochs):
        ...
        if val_ppl < best_val_ppl or math.isnan(val_ppl):
